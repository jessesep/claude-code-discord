...
- Predefined: 🎯 Manager, 💻 Coder, 🏗️ Architect, 🛡️ Security, 🧪 Tester, etc.
...
### Progress Bar (`agent/handlers.ts`)
Real-time elapsed time for long-running tasks:
- Updates every 3-5 seconds after initial delay
- Progress emoji: 🔄 → ⏳ → 🔄⏳ → ⏳⏳
- Footer shows "Working..." and "This may take a while"
- Automatically cleaned up on completion/error

### Model Fallback Chain (`util/list-models.ts`)
Automatic retry with fallback models:
- `MODEL_FALLBACK_CHAINS` - Same-gen fallbacks, never older
- `executeWithFallback()` - Auto-retry wrapper
- `shouldTriggerFallback()` - Detects 429, 503, 404 errors
- Supports Gemini 3/2.5/2.0, Claude, OpenAI families
...
- [x] Add progress bar for long-running tasks (#143) ✅
- [x] Implement model fallback chain (#145) ✅
- [ ] Add agent selection dropdown menu (#144)
- [ ] Add usage analytics dashboard (#146)
- [ ] E2E Test Suite Stability (#126)
