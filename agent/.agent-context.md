...
### Progress Bar for Long-Running Tasks (#143)
Located in `agent/handlers.ts`:
```typescript
// Start progress updates before agent execution
startProgressUpdates();
try {
  const result = await executeAgent();
} finally {
  stopProgressUpdates(); // ALWAYS cleanup
}
```

Key points:
- Updates Discord embed every 3-5 seconds with elapsed time
- Progress emoji escalates: 🔄 → ⏳ → 🔄⏳ → ⏳⏳
- MUST call `stopProgressUpdates()` on success, error, AND completion

### Model Fallback Chain (#145)
Located in `util/list-models.ts`:
```typescript
