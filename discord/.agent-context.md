...
- **UI Components**: Prefer embeds with buttons for structured output (e.g., git status, file lists).
 - **Command registration**: avoid destructive registration patterns at runtime.

## ⚠️ Known Operational Gotchas
- **Do not clear global commands by default**: current startup clears application global commands (tracked in issue **#67**).
- **Truncation/expand UX**: there is an open bug for expand buttons / truncated message UX (issue **#61**).
- **Registered commands vs handlers**: runtime registers a simplified set of commands; many handlers still exist but are not deployed unless explicitly added.

## 💡 Code Example: Interaction Context
```typescript
